...
class RelevancePropagationAvgPool2d(nn.Module):
    ...
    def __init__(
        self, layer: torch.nn.AvgPool2d,layer_name=None, eps: float = 1.0e-05, top_k: float = 0.0
    ) -> None:
        super().__init__()
        self.layer = layer
        self.layer_name = layer_name
...
class RelevancePropagationConv2d(nn.Module):
    ...
    def forward(self, a: torch.tensor, r: torch.tensor) -> torch.tensor:
        ...
        if hasattr(self, "accumulator") and self.accumulator is not None:
            self.accumulator.update(self.layer_id, r)
        z = self.layer.forward(a) + self.eps
        s = (r / z).data
        (z * s).sum().backward()
        c = a.grad
...
class RelevancePropagationFlatten(nn.Module):
    ...
    def __init__(self, layer: torch.nn.Flatten,layer_name=None, top_k: float = 0.0) -> None:
        super().__init__()
        self.layer = layer
        self.layer_name = layer_name
...
class RelevancePropagationReLU(nn.Module):
    ...
    def __init__(self, layer: torch.nn.ReLU,layer_name=None, top_k: float = 0.0) -> None:
        super().__init__()
        self.layer_name = layer_name
...
class RelevancePropagationDropout(nn.Module):
    ...
    def __init__(self, layer: torch.nn.Dropout, layer_name=None, top_k: float = 0.0) -> None:
        super().__init__()
        self.layer_name = layer_name
...
class RelevancePropagationBatchNorm2d(nn.Module):
    """Layer-wise relevance propagation for BatchNorm2d.

    Passes the relevance scores without modification.
    """

    def __init__(self, layer: torch.nn.BatchNorm2d,layer_name=None, top_k: float = 0.0) -> None:
        super().__init__()
        self.layer = layer
        self.layer_name = layer_name
...
class RelevancePropagationIdentity(nn.Module):
    ...
    def __init__(self, layer: nn.Module, layer_name=None, top_k: float = 0.0) -> None:
        super().__init__()
        self.layer_name = layer_name
